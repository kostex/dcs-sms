...
- [ ] **CA bundle fallback (me-mod ≥ 0.27.4):** rename `Saved Games/DCS/dcs-sms/lib/cacert.pem` → Refresh still syncs (it falls back to the copy in `<DCS>/MissionEditor/modules/dcs_sms_me/cacert.pem`). Rename that one too → Refresh reports `CA bundle missing (dcs-sms\lib\cacert.pem) — re-run "dcs-sms install-me-mod"` (never `error loading CA locations ((null))`), `dcs.log` lists every path tried, and the editor stays responsive. Restore both → Refresh syncs again.
