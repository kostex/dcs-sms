...
- **A waypoint's `type` and `action` are one setting, not two.** The `.miz` stores them as two strings, but in memory the ME keeps the *pair* in `wpt.type` (a reference into `panel_route.actions`, `{type=, name=, action=}`) and deletes `wpt.action` — `fixWaypointForGroup` does that on every load. ED's save then reads `wpt.type.type` / `wpt.type.action` and never looks at `wpt.action`. So any code that writes one half has to re-derive `wpt.type` from the pair (route_verbs' `apply_wp_mode`, which routes it through ED's own `waypointActionToType`); writing a bare string there serializes as `type=nil/action=nil` and the post-save reload dies in `me_route.lua:2414`. The verbs handle this for you — the `type`/`action` you pass and read back are always flat strings — but remember it if you drop to `exec --target gui`. Ground formations (Off Road, On Road, Cone, Rank, …) live in the **action**, with `type` staying `"Turning Point"`.
- **Vehicle groups need `route.spans`, and ED's own code is inconsistent about it.** `create_group` gives every `vehicle` group a per-leg road-network table. `insert_waypoint` then table.inserts into it with no nil guard (`me_mission.lua:6240`), so a hand-built vehicle group *without* one crashes `waypoint add` — after the waypoint is already in `route.points`, leaving a half-built route. `remove_waypoint` is the mirror image: it *is* nil-guarded, but when the table exists its index arithmetic runs off both ends (`spans[0]` removing the first waypoint, `points[0]` emptying the route), and it throws after `mapObjects` has been spliced but before `route.points` has — desyncing the group. `verb_helpers.inject_group` creates the table; `route_verbs` detaches it around `remove_waypoint` and regenerates after. If you build a group or call these primitives yourself, do the same.
...
| `community_ca.lua` | Finds a readable `cacert.pem` for the transport: the LuaSec payload in `dcs-sms\lib\` first, then the copy `install-me-mod` drops next to the mod itself. Probing before handing the path to LuaSec is deliberate — OpenSSL reports an unopenable CA file as a system error, whose reason string LuaSec prints as the useless `(null)`. |
...
| `dcs_sms_me/verbs/route_verbs.lua` | All `route_*`, `waypoint_*` verbs, plus the route-block locals: mode tables (`CATEGORY_DEFAULTS`, `WAYPOINT_TYPES/ACTIONS/MODES`, `AIRFIELD_TYPES`, `TYPE_CANONICAL_ACTION`, `ACTION_CANONICAL_TYPE`), the waypoint-mode helpers (`wp_type_action` reads the flat pair, `normalize_wp_type` resolves it through ED's converter, `apply_wp_mode` writes both halves, `flatten_wp` renders one for the wire), and the ME-state guards (`ensure_map_objects`, `ensure_route_spans`, `rebuild_route_spans`, `remove_waypoint_native`, `refresh_route_panel`, `find_route`, `find_waypoint`). Also home to `unit_set_parking` — it depends on `AIRFIELD_TYPES`, `ensure_map_objects`, and `refresh_route_panel`, so it's co-located with those rather than threading them through `verb_helpers.lua`. |
